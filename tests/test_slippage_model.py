...
def _seed(db, user_id: int, ticker: str, side: str, slippage_pct: float, shares: int, n: int = 1) -> None:
    for _ in range(n):
        db.save_slippage(
            user_id, ticker=ticker, expected_price=100.0,
            filled_price=100.0 * (1 + slippage_pct / 100),
            shares=shares, side=side, order_type="market",
        )


def test_empty_db_returns_default_globals(db, test_user_id):
    fit = fit_slippage(db)
    # No data → defaults shouldn't be zero (that would be a free lunch).
    assert fit.estimate_bps("NVDA", "buy", 100) >= 5.0


def test_per_ticker_estimate_uses_ticker_mean_when_enough_samples(db, test_user_id):
    # 15 buys at +3% (= 300 bps cost) for NVDA — enough to cross MIN_SAMPLES.
    _seed(db, test_user_id, "NVDA", "buy", 3.0, 100, n=MIN_SAMPLES + 5)
    fit = fit_slippage(db)
...
def test_unseen_ticker_falls_back_to_global_mean(db, test_user_id):
    _seed(db, test_user_id, "AAPL", "buy", 2.0, 100, n=MIN_SAMPLES + 5)  # 200 bps
    fit = fit_slippage(db)
...
def test_sell_slippage_sign_flip(db, test_user_id):
    # Sells that filled below expected → slippage_pct is negative but the
    # cost to us is positive bps.
    _seed(db, test_user_id, "NVDA", "sell", -1.5, 100, n=MIN_SAMPLES + 2)  # -1.5% = 150 bps cost
    fit = fit_slippage(db)
    est = fit.estimate_bps("NVDA", "sell", 100)
    assert 100 <= est <= 200


def test_larger_orders_inflate_estimate(db, test_user_id):
    # Fit a ticker at median 100 shares.
    _seed(db, test_user_id, "NVDA", "buy", 1.0, 100, n=MIN_SAMPLES + 5)  # 100 bps
    fit = fit_slippage(db)
